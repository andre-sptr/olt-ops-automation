...
def nama_file_ss():
    return os.path.join(FOLDER_GAMBAR, f"Funneling_OLT_{tanggal_hari_ini()}.png")


def nama_file_log():
    return os.path.join(FOLDER_LOG, "olt_log.txt")
...
async def ambil_screenshot():
    """Mengambil screenshot dari Google Spreadsheet dengan ukuran statis/tetap."""
    path_ss = nama_file_ss()
    catat_log(f"Mulai mengambil screenshot: {tanggal_hari_ini()}")

    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
        context = await browser.new_context(viewport={"width": 1920, "height": 1500})
        page = await context.new_page()

        target_url = URL_SPREADSHEET.replace(
            "/edit?usp=sharing",
            f"/htmlembed?gid={GID_SHEET}&widget=false&headers=false&chrome=false"
        )
        ...
        try:
            ...
            await page.screenshot(
                ...
                clip={
                    ...
                    "y": KORDINAT_Y,
                    "width": LEBAR_GAMBAR,
                    "height": TINGGI_GAMBAR
                }
            )
            catat_log("Screenshot statis berhasil diambil!")
...
def kirim_via_whatsapp(path_gambar):
    ...
    try:
        ...
        payload = {
            ...
            "file": {
                "mimetype": "image/png",
                "filename": f"Funneling_OLT_{tanggal_hari_ini()}.png",
                "data": encoded_string
            },
            "caption": f"📊 *Funneling OLT Down*\n📝 Created by ISP.SBT\nTanggal: {tanggal_hari_ini()}"
        }
...
def tugas_harian():
    ...
    catat_log("=" * 50)
    catat_log("🚀 Memulai tugas harian OLT...")
    ...
    catat_log("🏁 Tugas harian OLT selesai.")
    catat_log("=" * 50)


if __name__ == "__main__":
    print("=" * 55)
    print("  📱 Auto Screenshot & Kirim WA (OLT)")
