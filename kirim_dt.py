...
def nama_file_ss():
    return os.path.join(FOLDER_GAMBAR, f"Funneling_DT_{tanggal_hari_ini()}.png")


def nama_file_log():
    return os.path.join(FOLDER_LOG, "dt_log.txt")
...
def kirim_via_whatsapp(path_gambar):
    ...
    catat_log(f"Mengirim gambar ke grup WhatsApp via WAHA")
    ...
    try:
        ...
        payload = {
            ...
            "file": {
                "mimetype": "image/png",
                "filename": f"Funneling_DT_DH_{tanggal_hari_ini()}.png",
                "data": encoded_string
            },
            "caption": f"📊 *Funneling DT/DH SBT*\n📝 Created by ISP.SBT\nTanggal: {tanggal_hari_ini()}"
        }
...
def tugas_harian():
    ...
    catat_log("=" * 50)
    catat_log("🚀 Memulai tugas harian...")
