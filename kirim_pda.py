...
def nama_file_ss():
    return os.path.join(FOLDER_GAMBAR, f"Reporting_PDA_{tanggal_hari_ini()}.png")


def nama_file_log():
    return os.path.join(FOLDER_LOG, "pda_log.txt")
...
async def ambil_screenshot():
    ...
    async with async_playwright() as p:
        ...
        context = await browser.new_context(viewport={"width": 3000, "height": 2500})
...
def kirim_via_whatsapp(path_gambar):
    ...
    try:
        ...
        caption_baru = (
            f"📊*REPORT ORDER PDA & MODIF INDIBIZ DISTRICT PEKANBARU & DUMAI*\n"
        ...
        payload = {
            ...
            "file": {
                "mimetype": "image/png",
                "filename": f"Reporting_PDA_{tanggal_hari_ini()}.png",
                "data": encoded_string
            },
...
def tugas_harian():
    """Alur utama program."""
    catat_log("=" * 50)
    catat_log("🚀 Memulai tugas harian PDA...")
    ...
    catat_log("🏁 Tugas harian PDA selesai.")
    catat_log("=" * 50)


if __name__ == "__main__":
    print("=" * 55)
    print("  📱 Auto Screenshot & Kirim WA (PDA)")
