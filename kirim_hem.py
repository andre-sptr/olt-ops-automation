...
GID_SHEET = "2050842587"
RENTANG_CELL = "AC3:AM28"
...
def nama_file_ss():
    return os.path.join(FOLDER_GAMBAR, f"Reporting_HEM_{tanggal_hari_ini()}.png")


def nama_file_log():
    return os.path.join(FOLDER_LOG, "hem_log.txt")
...
def kirim_via_whatsapp(path_gambar):
    ...
    try:
        ...
        bulan_sekarang = nama_bulan_indonesia(datetime.now().month)
        caption_baru = (
            f"📊*Detail GOLIVE Project HEM {bulan_sekarang} | TIF SBT*\n"
            f"District Pekanbaru - Dumai\n"
            f"📝Created by ED.SBT\n"
        ...
        payload = {
            ...
            "file": {
                "mimetype": "image/png",
                "filename": f"Reporting_HEM_{tanggal_hari_ini()}.png",
                "data": encoded_string
            },
...
def tugas_harian():
    """Alur utama program."""
    catat_log("=" * 50)
    catat_log("🚀 Memulai tugas harian HEM...")
    ...
    catat_log("🏁 Tugas harian HEM selesai.")
    catat_log("=" * 50)


if __name__ == "__main__":
    print("=" * 55)
    print("  📱 Auto Screenshot & Kirim WA (HEM)")
