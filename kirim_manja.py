...
SPREADSHEET_ID = "1eKNaKPeaexBmGwHSRXC7yMo3wuI3DWJcC7zv73GkUMY"
GID_SHEET = 577446073
...
# ── TABLE CONFIGS ────────────────────────────────────────────────────
# Alarm TTR 3 Jam Manja  : kolom O4:R  → tampil O(Tiket), Q(Booking), P(STO), R(Distrik)
# Alarm TTR 6 Jam P      : kolom U4:X  → tampil V(Tiket), W(Booking), U(STO), X(Distrik)
# Alarm Tiket Open >24 Jam: kolom AA4:AD → tampil AB(Tiket), AA(STO), AC(Umur), AD(Distrik)
TABLE_CONFIGS = {
    "MANJA3JAM": {
        "key": "MANJA3JAM",
        "title": "Alarm TTR 3 Jam Manja",
        "start_row": 5,
        "display_cols": ["O", "Q", "P", "R"],
        "district_col": "R",
        "header": "Tiket | Jam Booking | STO | Distrik",
        "send_inti": True,
        # Kolom 1 = Jam Booking (Q) berubah terus; jangan dipakai untuk hash.
        "volatile_col_idx": 1,
    },
    "PLATINUM6JAM": {
        "key": "PLATINUM6JAM",
        "title": "Alarm TTR 6 Jam P",
        "start_row": 5,
        "display_cols": ["V", "W", "U", "X"],
        "district_col": "X",
        "header": "Tiket | Jam Open | STO | Distrik",
        "send_inti": True,
        # Kolom 1 = Jam Open (W) berubah terus; jangan dipakai untuk hash.
        "volatile_col_idx": 1,
    },
    "OPEN24JAM": {
        "key": "OPEN24JAM",
        "title": "Alarm Tiket Open > 24 Jam",
        "start_row": 5,
        "display_cols": ["AB", "AA", "AC", "AD"],
        "district_col": "AD",
        "header": "Tiket | STO | Umur (Jam) | Distrik",
        "send_inti": True,
        # Kolom 2 = Umur/Jam (AC) berubah terus; jangan dipakai untuk hash.
        "volatile_col_idx": 2,
    },
}
...
def nama_file_log():
    return os.path.join(FOLDER_LOG, "kirim_manja2_log.txt")
...
def buat_pesan_data(cfg, judul_suffix, rows):
    ...
    lines.extend(f"{i}. {' | '.join(row)}" for i, row in enumerate(rows, 1))
    return "\n".join(lines)
...
def baris_identitas(cfg, rows):
    """Buang kolom volatile (mis. "Jam Booking" / "Umur (Jam)") sebelum hash.
...
def buat_pesan_inti(cfg, suffix, rows_with_distrik):
    ...
    for distrik_norm in TARGET_DISTRIK:
        ...
        for display_row in rows:
            baris_tiket.append(f"{nomor}. {' | '.join(display_row)}")
            nomor += 1
    ...
    if unknown:
        ...
        for display_row in unknown:
            baris_unknown.append(f"{nomor}. {' | '.join(display_row)}")
            nomor += 1
        blocks.append("\n".join(baris_unknown))
...
def main():
    ...
    catat_log("Program Kirim Manja2 aktif. Memulai polling Google Sheet.")
